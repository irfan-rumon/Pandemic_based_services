...
class Nurse(Document):
	...
	certification_experience = StringField(max_length=100)
	charge = IntField()
...
class NurseDb():
	...
	def addnurses(self, image, name, phone, area, certification_experience,charge):
		nurse = Nurse.objects(pk = phone )

		if nurse:
			return {"success": True}
		else:
			new_nurse = Nurse(image=image, name=name, phone=phone, area=area, certification_experience=certification_experience,charge=charge)
			new_nurse.save()
			recent_added_nurse = Nurse.objects(pk = phone )
	...
	def get_single_nurse(self, phone):
		nurses = Nurses.objects(pk = phone)
		nurse_info = [] 
		if nurses:
			nurse = nurses[0]
			nurseDict = {                              
	...
	def add_comment(self, nurse_phone, user_phone, user_name, commnent): 
	...
	def get_all_nurse_comments(self, nurse_phone):
		comments = Nurse_comment.objects( pk = nurse_phone )
		commentList = []
		if len(comments) > 0:                                 			                                            
